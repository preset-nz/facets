...
// The licence file only exists after a build. In dev, serve the last built
// one, or a note, instead of falling through to index.html.
const LICENCES = fileURLToPath(new URL("./dist/licenses.txt", import.meta.url))
const devLicences = (): Plugin => ({
  name: "dev-licences",
  apply: "serve",
  configureServer(server) {
    server.middlewares.use("/licenses.txt", (_req, res) => {
      ...
      res.end(
        existsSync(LICENCES)
          ? readFileSync(LICENCES, "utf8")
          : "Generated by the production build. Run `just playground-build`, then reload.\n",
      )
    })
  },
})
